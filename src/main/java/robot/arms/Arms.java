...
public class Arms extends SubsystemBase {
  ...
  private final PIDController pid =
      new PIDController(ArmsConstants.kP, ArmsConstants.kI, ArmsConstants.kD);
  ...
  public Arms(ArmsIO hardware) {
    ...
    pid.setTolerance(
        ArmsConstants.MAX_ANGLE.in(edu.wpi.first.units.Units.Radians) * ArmsConstants.Tolerance,
        0.2);
  ...
  /**
   * there will not be a need for the boolean ClampInput by binding directly to the button action.
   * Usually put this in where you specifify the button input
   *
   * @param clampInput
   * @return
   */
  ...
  public Command clampRelease() {
    return run(() -> hardware.setVoltage(ArmsConstants.STARTING_VOLTAGE));
  }
